...
# import video rollout manager
# from vagen.rollout.qwen_rollout.rollout_manager_video import QwenVLVideoRolloutManager
...
class RayPPOTrainer(object):
    ...
    def _create_dataloader(self):
        ...
        print(f"[DEBUG] ray_trainer.py: val_batch_size: {val_batch_size}")
    ...
    def _maybe_log_val_generations_to_wandb(self, log_rst: List[Dict[str, Any]]):
        ...
        for item in log_rst:
            ...
            # 0609: temporarily disable image logging
            images.append(None)
    ...
    def _save_checkpoint(self):
        ...
        # ADD: chose to save critic or not
        # TODO: add a config to choose to save critic or not
        save_critic=True #False
        if self.use_critic and save_critic:
    ...
    def fit(self):
        ...
        if self.config.rollout_manager.get("use_service",False):
            ...
            #rollout_manager = QwenVLVideoRolloutManager(
                actor_rollout_wg=self.actor_rollout_wg,
                config=self.config.rollout_manager,
                tokenizer=self.tokenizer,
                processor=self.processor,
