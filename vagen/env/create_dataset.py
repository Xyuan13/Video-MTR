...
def create_dataset_from_yaml(yaml_file_path: str, force_gen=False,seed=42,train_path='./train.parquet',test_path='./test.parquet'):
    ...
    for key, value in yaml_config.items():
        ...
        if hasattr(env_config, 'generate_seeds'):
            # Special handling for video environment that needs split parameter
            if env_name == 'video':
                seeds_for_env_train = env_config.generate_seeds(train_size, split="train")
                seeds_for_env_test = env_config.generate_seeds(test_size, split="test")
            else:
